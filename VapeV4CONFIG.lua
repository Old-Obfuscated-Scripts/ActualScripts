-- HOLY FUCKING SHIT I SPENT SO LONG TRYING TO FIND THIS LAST YEAR WOO! 

print("Loading")
local loadtime = tick()
local ScriptSettings = {Crosshair = false, MageAnimation = false, SlowAutoWin = false, Night = false}
...
runFunction(function()
 local Main213 = GuiLibrary.ObjectsThatCanBeSaved
end)
...
runFunction(function()
	...
	local anims = {
		Block = {
			{CFrame = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(-30), math.rad(50), math.rad(-90)), Time = 0.00001} -- skidded from vapes exhi anim 🤓 (says the skid)
		},
		Normal = {
...
local fakeplr = {["Name"] = "Yourself dumbass", ["UserId"] = "239702688"}
local otherfakeplayers = {["Name"] = "Dumbasses", ["UserId"] = "1"}
	...
	runFunction(function()
		...
		function tweenToNearestBed()
			...
			if nearestBed and not hasTeleported then
				...
				local tween = TweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.96), {CFrame = nearestBed.CFrame + Vector3.new(0, 3, 0)})
				tween:Play()
			end
		end
...
runFunction(function()
		ThirdPack = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
			Name = "Texturepack",
			Function = function(callback)
				if callback then 
...
runFunction(function()
    ...
    Disabler = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		HoverText = "Have fun! 50+ speed ez"
    })
end)
...
local OldAntiVoid = {["Enabled"] = false}
      OldAntiVoid = GuiLibrary["ObjectsThatCanBeSaved"]["WorldWindow"]["Api"].CreateOptionsButton({
    ["Name"] = "Stud Antivoid",
    ["Function"] = function(callback) 
        if callback then
...
runFunction(function()
	...
	themeselected = GameThemeV2.CreateDropdown({
		["Name"] = "Theme",
		["Function"] = function() end,
		["List"] = {"NebulaSky", "PinkMountainSky", "CitySky", "PinkSky", "EgirlSky", "SpaceSky"}
	})
end)
runFunction(function()
...
runFunction(function()
	...
	ChinaHat = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		...
		HoverText = "Puts a china hat on your character (Made by mastadawn)"
	})
...
local whitelist = game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/Tropxzz/Scripts/main/Skidclientwhitelist.lua"))
		...
		if whitelist["Owner"] ~= nil then
			for i, v in pairs(whitelist["Owner"]) do
				if v.id == userId then
					if not hasTag then
						hasTag = true
						p.PrefixText = "<font color='#ff0000'>[SKID CLIENT OWNER (ANTI SKID)]</font> " .. msg.PrefixText
					end
					userType = 3
				end
			end
...
runFunction(function()
    ydubas = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
        Name = "word repeater",
        Function = function(callback)
            if callback then
                game.TextChatService.MessageReceived:Connect(function(m)
                    local lplr = game.Players.LocalPlayer
                    for _, recipientPlayer in pairs(game.Players:GetPlayers()) do
                        if recipientPlayer ~= lplr then
                            game:GetService("TextService"):SendAsync(lplr, recipientPlayer, m.Text)
                        end
                    end
                end)
            end
...
runFunction(function()
	Glizzy = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "Glizzy",
		Function = function(callback)
			if callback then --// services
				local Players = game:GetService("Players")
				local ReplicatedStorage = game:GetService("ReplicatedStorage")
				local Workspace = game:GetService("Workspace")
...
runFunction(function()
	...
	Dupe = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		Name = "Dupe (broken)",
		Function = function(callback) 
			if callback then
...
runFunction(function()
GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
Name = "WASD and space ui (made by me)",
Function = function(callback)
 if callback then
	local ScreenGui = Instance.new("ScreenGui")
	...
	Space.BackgroundColor3 = Color3.fromRGB(0, 213, 255)
	Space.BackgroundTransparency = 0.500
	Space.BorderColor3 = Color3.fromRGB(0, 0, 0)
	Space.BorderSizePixel = 0
	Space.Position = UDim2.new(0.0597976074, 0, 0.928730488, 0)
	...
	UICorner.Parent = Space
	
	W.Name = "W"
	W.Parent = ScreenGui
	W.BackgroundColor3 = Color3.fromRGB(0, 213, 255)
	W.BackgroundTransparency = 0.500
	W.BorderColor3 = Color3.fromRGB(0, 0, 0)
	W.BorderSizePixel = 0
	W.Position = UDim2.new(0.0993560255, 0, 0.712694883, 0)
	W.Size = UDim2.new(0, 113, 0, 50)
	...
	A.Name = "A"
	A.Parent = ScreenGui
	A.BackgroundColor3 = Color3.fromRGB(0, 213, 255)
	A.BackgroundTransparency = 0.500
	A.BorderColor3 = Color3.fromRGB(0, 0, 0)
	A.BorderSizePixel = 0
	A.Position = UDim2.new(0.0340386368, 0, 0.783964336, 0)
	A.Size = UDim2.new(0, 113, 0, 50)
	...
	S.Name = "S"
	S.Parent = ScreenGui
	S.BackgroundColor3 = Color3.fromRGB(0, 213, 255)
	S.BackgroundTransparency = 0.500
	S.BorderColor3 = Color3.fromRGB(0, 0, 0)
	S.BorderSizePixel = 0
	S.Position = UDim2.new(0.164673418, 0, 0.783964336, 0)
	S.Size = UDim2.new(0, 113, 0, 50)
	...
	D.Name = "D"
	D.Parent = ScreenGui
	D.BackgroundColor3 = Color3.fromRGB(0, 213, 255)
	D.BackgroundTransparency = 0.500
	D.BorderColor3 = Color3.fromRGB(0, 0, 0)
	D.BorderSizePixel = 0
	D.Position = UDim2.new(0.0993560255, 0, 0.860801756, 0)
	D.Size = UDim2.new(0, 113, 0, 50)
	...
	local function SRCCOSX_fake_script() -- ScreenGui.LocalScript 
		local script = Instance.new('LocalScript', ScreenGui)
	
		local asdajhk21hkj3h12kj3h = game:GetService("UserInputService")
		
		local keyToElement = {
			[Enum.KeyCode.W] = script.Parent.W,
			[Enum.KeyCode.Space] = script.Parent["S[pace"],
			[Enum.KeyCode.S] = script.Parent.S,
			[Enum.KeyCode.D] = script.Parent.D,
			[Enum.KeyCode.A] = script.Parent.A
		}
		
		for key, element in pairs(keyToElement) do
			asdajhk21hkj3h12kj3h.InputBegan:Connect(function(input)
				if input.KeyCode == key then
					element.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
				end
			end)
		
			asdajhk21hkj3h12kj3h.InputEnded:Connect(function(input)
				if input.KeyCode == key then
					element.BackgroundColor3 = Color3.fromRGB(0, 213, 255)
				end
			end)
		end
		
	end
	coroutine.wrap(SRCCOSX_fake_script)()	
 end 
end
})
end)
...
runFunction(function()
    GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
        Name = "Moons Texture pack (coded by me :))",
        Function = function(callback)
            if callback then
                local Players = game:GetService("Players")
                local ReplicatedStorage = game:GetService("ReplicatedStorage")
                local Workspace = game:GetService("Workspace")
                local objs = game:GetObjects("rbxassetid://14380686814")
                local import = objs[1]
                import.Parent = ReplicatedStorage
                ...
                local function applyTransparencyToDescendants(part)
                    if part:IsA("Part") or part:IsA("MeshPart") or part:IsA("UnionOperation") then
                        part.Transparency = 1
                    end
                end

                local function applyModelToTool(tool, modelInfo)
                    for _, v in pairs(tool:GetDescendants()) do
                        applyTransparencyToDescendants(v)
                    end

                    local model = modelInfo.model:Clone()
                    model.CFrame = tool:WaitForChild("Handle").CFrame * modelInfo.offset
                    model.CFrame *= CFrame.Angles(math.rad(0), math.rad(-50), math.rad(0))
                    model.Parent = tool
                ...
                local function onToolAdded(tool)
                    ...
                    for _, v in pairs(index) do
                        if v.name == tool.Name then
                            applyModelToTool(tool, v)
                        end
                    end
                end

                local function applyModelToLocalPlayer(tool, modelInfo)
                    local tool2 = Players.LocalPlayer.Character:WaitForChild(tool.Name)
                    for _, v in pairs(tool2:GetDescendants()) do
                        applyTransparencyToDescendants(v)
                    end

                    local model2 = modelInfo.model:Clone()
                    model2.Anchored = false
                    model2.CFrame = tool2:WaitForChild("Handle").CFrame * modelInfo.offset
                    model2.CFrame *= CFrame.Angles(math.rad(0), math.rad(-50), math.rad(0))
                ...
                local toolAddedConnection = Workspace:WaitForChild("Camera").Viewmodel.ChildAdded:Connect(onToolAdded)
...
runFunction(function()
	local hihi
	local viewmodelchams = {Enabled = false}
	local viewmodelchamsfilltransparency = {Value = 0.5}
	local viewmodelchamsfillcolor = {
		Hue = 0,
		Sat = 0,
		Val = 0
	}
	local viewmodelchamsoutlinetransparency = {Value = 0}
	local viewmodelchamsoutlinecolor = {
		Hue = 0,
		Sat = 0,
		Val = 0
	}
	viewmodelchams = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "ViewmodelChams",
		HoverText = "Made by dev @iraqicat",
		Function = function(callback)
			if callback then
				for i1,v1 in pairs(game.ReplicatedStorage.Items:GetChildren()) do
					for i2,v2 in pairs(workspace.Camera:WaitForChild("Viewmodel"):GetChildren()) do
	...
	viewmodelchamsfilltransparency = viewmodelchams.CreateSlider({
		Name = "Fill transparency",
		Min = 0,
		Max = 100,
		Default = 50,
		Function = function(val)
			if viewmodelchams.Enabled then
				for i,v in pairs(workspace.Camera:WaitForChild("Viewmodel"):GetDescendants()) do
					if v:IsA("Highlight") then
	...
	viewmodelchamsfillcolor = viewmodelchams.CreateColorSlider({
		Name = "Fill Color",
		Function = function(h, s, v) 
			viewmodelchamsfillcolor.Hue = h
			viewmodelchamsfillcolor.Sat = s
			viewmodelchamsfillcolor.Val = v
			if viewmodelchams.Enabled then
				for i,v in pairs(workspace.Camera:WaitForChild("Viewmodel"):GetDescendants()) do
					if v:IsA("Highlight") then
						v.FillColor = Color3.fromHSV(viewmodelchamsfillcolor.Hue,viewmodelchamsfillcolor.Sat,viewmodelchamsfillcolor.Val)
					end
				end
			end
		end
	})
	viewmodelchamsoutlinetransparency = viewmodelchams.CreateSlider({
	...
	viewmodelchamsoutlinecolor = viewmodelchams.CreateColorSlider({
		Name = "Outline Color",
		Function = function(h, s, v) 
			viewmodelchamsoutlinecolor.Hue = h
			viewmodelchamsoutlinecolor.Sat = s
			viewmodelchamsoutlinecolor.Val = v
			if viewmodelchams.Enabled then
				for i,v in pairs(workspace.Camera:WaitForChild("Viewmodel"):GetDescendants()) do
					if v:IsA("Highlight") then
						v.OutlineColor = Color3.fromHSV(viewmodelchamsoutlinecolor.Hue,viewmodelchamsoutlinecolor.Sat,viewmodelchamsoutlinecolor.Val)
					end
				end
			end
		end
...
runFunction(function()
	Watermark = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "Watermark (fk velocity)",
		Function = function(callback)
			if callback then
				-- Your code to create the watermark goes here
				local url = "https://pastebin.com/raw/KjPUtjX7"
...
runFunction(function()
	Rain = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "Rain",
		Function = function(callback)
			if callback then
	...
	runFunction(function()
		tpem = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
			Name = "EmeraldTP",
			Function = function(callback)
				if callback then
	...
	runFunction(function()
	tpdim = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		Name = "DiamondTP",
		Function = function(callback)
			if callback then
				lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Dead)
...
runFunction(function()
	Keystrokes = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "Keystrokes",
		Function = function(callback)
			if callback then
...
runFunction(function()
	...
	BoostAirJump = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
		...
		["HoverText"] = "Azura Client Private is crazy"
	})
end)
print("13098 loaded")

runFunction(function()
	local AnticheatDisabler = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
...
runFunction(function()
	local MageAnimation = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		["Name"] = "MageAnimation",
		["Function"] = function(callback)
			if callback then
				pcall(function()
...
runFunction(function()
	local SlowAutoWin = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		["Name"]  = "SlowAutoWin",
...
runFunction(function()
	local targetIsAlive = function()
		for i,v in pairs(shared.VapeTargetInfo.Targets) do
			return true
		end
	end

	local dashDisabler = {Enabled = false}
	local speedChanger = {Value = 28}
	dashDisabler = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		Name = "AnticheatBypass",
		HoverText = "(re-enable with scythe to bypass) firewallbypass but better (dont use it on japan server)",
		ExtraText = function() return tostring(speedChanger.Value).."CFrame" end,
		Function = function(callback)
			if callback then
				task.spawn(function()
					repeat
						local item = getItemNear("scythe")
						local scytheDashArgs = {
							[1] = {
								["direction"] = Vector3.new(9e9, 9e9, 9e9)
							}
						}
						task.wait()
						if item and lplr.Character.HandInvItem.Value == item.tool then
							game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.ScytheDash:FireServer(unpack(scytheDashArgs))
						...
						task.wait()
					until not dashDisabler.Enabled
				end)
			else
				task.wait(0.05)
				bedwarsStore.scythe = tick()
	...
	speedChanger = dashDisabler.CreateSlider({
		Name = "Speed",
		Min = 23, 
		Max = 55, 
		Function = function(val)
			newSpeed = val - 23 --yawnin
		end, 
		Default = 28
	})
end)
...
runFunction(function()
	local detected = {}
	local function speedCheck(player)
		if player ~= lplr and player.Character and player.Character.PrimaryPart and player.Character.Humanoid and player.Character.Humanoid.Health > 0 then
			...
			repeat
				...
				task.wait(1)
				mag = (((Vector2.new(player.Character.PrimaryPart.Position.X,player.Character.PrimaryPart.Position.Z) - newPos).magnitude) * 1)
				if mag >= 21.1 then
					flagged = flagged + 1
				end
			until looped >= 4
			if flagged == 4 then
				...
				if not alreadyDetected and player.Character and player.Character.PrimaryPart and player.Character.Humanoid and player.Character.Humanoid.Health > 0 then
					warningNotification("Skid client AC (Speed)",player.Name.." is cheating. (Speed: "..tostring((math.round(mag * 10) / 10))..")",30)
					table.insert(detected, player)
				end
			end
		end
	end
	local function verticalCheck(player)
		if player ~= lplr and player.Character and player.Character.PrimaryPart and player.Character.Humanoid and player.Character.Humanoid.Health > 0 then
			local alreadyDetected = false
			if player.Character.PrimaryPart.Position.Y > 500 then
				for i,v in pairs(detected) do
					if v == player then 
						alreadyDetected = true
					end
				...
				if not alreadyDetected then
					warningNotification("Skid client AC: (InfiniteFly)",player.Name.." is cheating. (YPos: "..tostring(math.round(player.Character.PrimaryPart.Position.Y))..")",60)
					table.insert(detected, player)
				end
			end
		end
	...
	lunarVapeAnticheat = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		Name = "Skid client AC",
		Function = function(callback)
			if callback then
...
runFunction(function()
    ...
    AntiDeath = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
        Name = "AntiDeath",
...
runFunction(function()
	local LoadingTime = {Enabled = false}
	local LoadingTimeNotification = {Value = 5}
	LoadingTime = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "LoadingTime",
        HoverText = "Notifies you on how long the loading time took",
		Function = function(callback)
			if callback then
				warningNotification("Skid client","Loaded in " .. tick() - loadtime .. " seconds.",LoadingTimeNotification.Value)
				return
			end
		end
	})
...
runFunction(function()
	...
	ViewModel = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "ViewModel",
        HoverText = "Customizes your items\nMade by melothefunny",
		Function = function(callback)
			if callback then
				local Players = game:GetService("Players")
				local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
runFunction(function()
	...
	Snowflakes = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "Snowflakes",
        HoverText = "Makes it snow",
		Function = function(callback)
			if callback then
				task.spawn(function()
					-- vape code yes ik, i jst made it separate
					local snowpart = Instance.new("Part")
					snowpart.Size = Vector3.new(240, 0.5, 240)
					snowpart.Name = "SnowParticle"
					snowpart.Transparency = 1
	...
	SnowflakesSpread = Snowflakes.CreateSlider({
		Name = "Spread",
		Min = 1,
		Max = 100,
		Function = function() end,
	...
	SnowflakesRate = Snowflakes.CreateSlider({
		Name = "Rate",
		Min = 1,
		Max = 100,
		Function = function() end,
...
print("Thank you: Piston, Snoopy, and other shit i stole from ppl this config is obfuscated/n since there are leaked shit make ppl lose there hard work/n because a dumb config is unobfuscated/n peace")
